...
public class ID3Compression {
    ...
    protected static ByteBuffer uncompress(String identifier, String filename, ByteBuffer byteBuffer, int decompressedFrameSize, int realFrameSize) throws InvalidFrameException {
        logger.info(filename + ":About to decompress " + realFrameSize + " bytes, expect result to be:" + decompressedFrameSize + " bytes");
        ...
        try {
            ...
            logger.info(filename + ":Decompressed to " + inflatedTo + " bytes");
        } catch (DataFormatException dfe) {
            logger.warn( "Unable to decompress this frame:" + identifier, dfe);
